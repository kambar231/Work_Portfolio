 ...
 *
 * LAYOUT (fixes round): cubes render ~90 px tall at 1440 wide with generous air
 * (spacing 2.4 units); the whole 4x2 group projects into the upper-right region and
 * never overlaps the headline or a label at 1280/1440/1920 (asserted by the projection
 * verifier). Scatter targets are a screen-centred expansion of the grid (jittered, with
 * depth) chosen by a seeded search so no two cubes overlap on screen at any point of the
 * unravel and every cube stays inside the view frustum.
 *
 * PUBLIC API:
...
const FOV = 45;
const CUBE_PX = 90;           // target on-screen height of a 1-unit cube (px)
const SPACING = 2.4;          // grid spacing in units (gaps > one cube width)
const REGION_NDC_X = 0.38;    // grid centre in NDC -> upper-right of the hero
const REGION_NDC_Y = 0.58;    // high enough that the bottom row + labels clear the headline
const LABEL_DROP = 0.62;      // how far below a cube its label sits (world units)
const BOB_AMP = 0.06;
const TUMBLE_X = 0.0022;      // per 1/60 s
const TUMBLE_Y = 0.0035;
...
export function createCubeHero({ onCubeClick } = {}) {
  ...
  for (let i = 0; i < CUBES.length; i++) {
    ...
    gridLocal.push(new THREE.Vector3((col - 1.5) * SPACING, (0.5 - row) * SPACING, 0));
    ...
    for (let f = 0; f < 6; f++) {
      ...
      const mat = new THREE.MeshStandardMaterial({ color: 0xbfbfbf, roughness: 0.7, metalness: 0 });
  ...
  function genScatter() {
    const EX = 1.5, EY = 2.15;                 // horizontal / vertical expansion
    const JIT = 0.7, ZMIN = -3, ZMAX = 0.5;
